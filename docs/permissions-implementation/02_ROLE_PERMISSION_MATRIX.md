...
هذه المصفوفة مطابقة حرفيًا لـ`CANONICAL_ROLE_PERMISSION_MAP` ومتحقق منها في قاعدة التطوير باختبار set equality.
...
يملك جميع المفاتيح الـ75 الواردة في الكتالوج النهائي. لا يملك تلقائيًا أيًا من قدرات workflow policy-only الخمس؛ الاستثناء يُمنح لمستخدم محدد من endpoint مستقل ويظهر في `policyCapabilities` لا `permissions`.
