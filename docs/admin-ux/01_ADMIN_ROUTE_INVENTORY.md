...
|---:|---|---|---|---|---|---|---|---|
| 1 | `/ar/admin` — dashboard | Operational | بطاقات عرض؛ لا | V + rebuild search | `dashboard.view`, `search.index.rebuild` للإجراء | `GET /admin/dashboard`, `POST /admin/reindex` | `AdminLayout`, `AdminPageHeader`, `SearchReindexAction` بتأكيد | مدخل تشغيلي مكتمل في #54؛ `admin-view-first`, visual |
...
| 3 | `/ar/admin/imports/queue` — import queue | Workflow | قائمة تشغيل وإجراءات؛ المدخلات للإجراء فقط | V/C/W | `source.view`, `source.upload`, `source.review`, `source.draft.create` | `GET/POST /imports`, `POST /imports/:id/review`, `POST /imports/:id/draft` | قائمة تشغيل + اختصار «رفع مصدر» + preview كبير + حوارات الإجراء القائمة | استثناء Workflow؛ `admin-ui`, `import-structure`, visual |
...
| 5 | `/ar/admin/content` — legislations | Entity/List | جدول بحث وفلترة؛ لا؛ كان `legislation.create` بلا زر | V/C/U direct؛ A عبر Workflow؛ لا D | `legislation.view/create/update/published_metadata.update/archive`; `source.upload` لاختصار الاستيراد | `GET /admin/legislations`, `POST /legislations`; التعديل/الأرشفة في مسارات التفاصيل | جدول قراءة + `AdminRowActions` + Add dialog + Search/Filter/Pagination؛ التعديل المباشر يفتح Dialog التفاصيل | **مكتمل في #54**؛ إنشاء/عرض/تعديل ظاهر ومختبر؛ visual |
...
| 13 | `/ar/admin/amendments/list` — amendments | Workflow/List | جدول وإجراءات مراجعة/نشر؛ لا | V/C/W | `amendment.view/create/review/publish` | `GET/POST /admin/amendments`, `POST /admin/amendments/:id/review|publish` | قائمة تشغيل، الإجراءات المتاحة فقط، واختصار «إنشاء تعديل» | استثناء Workflow؛ `admin-auth`, visual |
...
| 17 | `/ar/admin/users` — users | Entity/List | جدول قراءة لكن إنشاء inline | V/C/U/A enable/disable؛ لا D | `user.view/create/update/enable/disable`, `role.view` | `GET/POST /admin/users`, `PATCH /admin/users/:id/profile|state` | جدول + `AdminRowActions` للعرض/التعديل/التعطيل + Add dialog + `ConfirmDialog` للحالة | **مكتمل في #54**؛ `canManage` يطابق سقف السلطة وأزيل تحديد الصف من view-only؛ `admin-view-first`, `admin-auth`, visual |
| 18 | `/ar/admin/users/:id/profile` — user profile | Entity/Detail | Form دائم؛ **نعم** | V/U/A/reset | `user.view/update/enable/disable/reset_password` + سقف السلطة | `GET /admin/users/:id/access`, `PATCH /profile`, `PATCH /state`, `POST /reset-password` | `EntityDetails` + Edit/Reset dialogs + confirmations | **محول**؛ كل mutations مشروطة بـ`canManage`؛ `admin-view-first` |
...
| 24 | `/ar/admin/roles` — roles | Entity/List | جدول قراءة لكن إنشاء inline؛ كانت U/D داخل التفاصيل فقط | V/C/U/D للمخصص غير المحمي وغير المسند | `role.view/create/update/delete` + سقف السلطة | `GET/POST/PATCH/DELETE /admin/access-roles` | جدول + `AdminRowActions` + Add/Edit dialogs + delete confirmation | **مكتمل في #54**؛ `canManage` يحترم الحماية والإسناد وسقف السلطة؛ `admin-view-first`, visual |
| 25 | `/ar/admin/roles/:id/general` — role | Entity/Detail | Form دائم؛ **نعم** | V/U/D للمخصص غير المسند | `role.view/update/delete` + سقف السلطة | `GET/PATCH/DELETE /admin/access-roles/:id` | `EntityDetails` + Edit dialog + `ConfirmDialog` | **محول**؛ mutations مشروطة بـ`canManage`؛ `admin-view-first` |
...
| 29 | `/ar/admin/permissions/matrix` — catalog matrix | Entity/Reference | مصفوفة read-only؛ لا | V؛ الإدارة من الأدوار | `permission.view`, `role.view` لاختصار الإدارة | `GET /admin/permissions` | Matrix responsive متخصصة + إجراء واضح إلى الأدوار | كتالوج immutable؛ لا CRUD مصطنع؛ visual |
| 30 | `/ar/admin/permissions/resources` — catalog by resource | Entity/Reference | كتالوج read-only؛ لا | V؛ الإدارة من الأدوار | `permission.view`, `role.view` لاختصار الإدارة | `GET /admin/permissions` | `PermissionExplorer` catalog + إجراء إلى الأدوار | كتالوج immutable؛ لا CRUD مصطنع؛ visual |
| 31 | `/ar/admin/permissions/roles` — role summary | Entity/Reference | بطاقات read-only؛ لا | V؛ الإدارة من الأدوار | `permission.view`, `role.view` | `GET /admin/permissions` | بطاقات وروابط تفاصيل + إجراء إلى الأدوار | View First؛ منح الصلاحيات من الدور؛ visual |
| 32 | `/ar/admin/permissions/sensitive` — sensitive catalog | Entity/Reference | كتالوج read-only؛ لا | V؛ الإدارة من الأدوار | `permission.view`, `role.view` لاختصار الإدارة | `GET /admin/permissions` | `PermissionExplorer` filtered + إجراء إلى الأدوار | كتالوج immutable؛ لا CRUD مصطنع؛ visual |
| 33 | `/ar/admin/synonyms` — search synonyms | Entity/List | جدول؛ إنشاء inline وحذف مباشر | V/C/D/activate + rebuild search | `search.synonym.view/create/delete`, `search.synonym_set.activate`, `search.index.rebuild` | `GET/POST/DELETE /admin/synonyms`, `POST /admin/synonym-sets/:id/activate`, `POST /admin/reindex` | Add dialog + delete/activate confirmations + `SearchReindexAction` | **محسن**؛ اكتمل مدخل reindex في #54؛ `admin-view-first`, visual |
...
- إنشاء التشريع متاح بطريقتين واضحتين: Dialog لمسودة سجل أساسي عبر `POST /legislations` القائم، أو مسار الاستيراد المخصص عند وجود ملف مصدر. لا يضيف أي منهما hard delete للتشريع.
