...
### تحديث نسخة تطوير موجودة

بعد جلب تغييرات تحتوي ترحيلات، خذ نسخة احتياطية من قاعدة البيانات المحددة في `.env`، ثم نفّذ `npm run db:migrate` و`npm run db:check` قبل تشغيل `npm run dev`. لا تعِد تشغيل `seed` على بياناتك القائمة؛ فهو يعيد بيانات التطوير بالكامل.

إذا فشل الدخول بخطأ `Unknown column 'deleted_at' in 'WHERE'`، فالكود أحدث من مخطط القاعدة. تطبّق `npm run db:migrate` الترحيلات المعلقة، ومنها `1700000000015` و`1700000000016` لدورة حياة السجلات؛ لا يلزم تغيير كلمة المرور أو إعادة إنشاء الحسابات. افحص الدخول وفتح لوحة الإدارة واستمرار الجلسة بعد إعادة تحميل الصفحة عقب التحديث.
...
يفحص الخادم وعامل المهام توافق القاعدة قبل بدء الخدمة. لا يطبق هذا الفحص ترحيلات تلقائياً؛ يذكر الترحيلات أو الحقول المفقودة ويرفض قاعدة أحدث أو غير معروفة. يدعم سجل التطوير التاريخي `PermissionCatalogSync1700000000011` إلى جانب الترحيلات المطلوبة، ولا يحذف هذا السجل أو يعيد إسناد الصلاحيات. احتفظ بمجلد `scripts/database/` عند نشر الملفات المبنية.

استعادة الوصول تبدأ من `/ar/forgot-password` بطلب محفوظ باسم المستخدم. يعالج المسؤول صاحب `user.reset_password` الطلب من تبويب «طلبات الاستعادة» داخل المستخدمين، بعد التحقق من الهوية وكتابة السبب. يظهر رابط صالح لمدة 15 دقيقة مرة واحدة ليسلّمه المسؤول يدوياً؛ لا يرسل النظام رسالة تلقائية. إتمامه يبطل الجلسات السابقة. تظل قيود إدارة النفس والحسابات المساوية أو الأعلى في السلطة مطبقة.

عند تعديل المراجع أو الجريدة أو الصفحات أو التنقل أو الإعدادات، يرفض الخادم نسخة التحرير القديمة بالرمز `409`. تحتفظ الواجهة بالمدخلات وتعرض القيم الحالية للمراجعة قبل إعادة الحفظ. تحتاج استدعاءات API المخصصة إلى تمرير `editRevision` أو خريطة `editRevisions` المسترجعة من واجهة الإدارة.
