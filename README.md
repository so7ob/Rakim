...
بعد جلب تغييرات تحتوي ترحيلات، خذ نسخة احتياطية من قاعدة البيانات المحددة في `.env`، ثم نفّذ `npm run db:migrate` قبل تشغيل `npm run dev`. لا تعِد تشغيل `seed` على بياناتك القائمة؛ فهو يعيد بيانات التطوير بالكامل.
