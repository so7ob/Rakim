...
export class LegislationsService {
  ...
  async list(query: Record<string, string | undefined>) {
    ...
    const where = [`l.status IN ('PUBLISHED','AMENDED','REPEALED','SUSPENDED')`];
    const values: QueryValue[] = [];
    if (query.q?.trim()) {
      where.push(`EXISTS (SELECT 1 FROM search_documents sd WHERE sd.legislation_id = l.id AND sd.text_normalized LIKE ?)`);
      values.push(`%${normalizeArabic(query.q)}%`);
    }
    if (query.type) { where.push('lt.code = ?'); values.push(query.type); }
    if (query.authority) { where.push('au.code = ?'); values.push(query.authority); }
    if (query.status) { where.push('l.legal_status = ?'); values.push(query.status); }
    if (query.yearFrom) { where.push('l.year >= ?'); values.push(Number(query.yearFrom)); }
    if (query.yearTo) { where.push('l.year <= ?'); values.push(Number(query.yearTo)); }
    if (query.verification) { where.push('l.verification_level = ?'); values.push(query.verification); }
    const sortMap: Record<string, string> = {
      newest: 'l.year DESC, l.official_number DESC', oldest: 'l.year ASC, l.official_number ASC',
      number: 'CAST(l.official_number AS UNSIGNED), l.official_number', title: 'l.title_ar',
      reviewed: 'l.last_reviewed_at DESC', relevance: 'l.year DESC',
    };
    const order = sortMap[query.sort ?? 'newest'] ?? sortMap.newest;
    const filterSql = where.join(' AND ');
    const countRows = await this.db.query(`SELECT COUNT(*) total FROM legislations l JOIN legislation_types lt ON lt.id=l.type_id JOIN authorities au ON au.id=l.authority_id WHERE ${filterSql}`, values) as Array<{total:string}>;
    const items = await this.db.query(`
      ...
      WHERE ${filterSql} ORDER BY ${order} LIMIT ? OFFSET ?`, [...values, pageSize, (page - 1) * pageSize]);
    const [types, authorities, years] = await Promise.all([
      this.db.query('SELECT code, name_ar name FROM legislation_types WHERE is_active=1 ORDER BY name_ar'),
      this.db.query('SELECT code, name_ar name FROM authorities WHERE is_active=1 ORDER BY name_ar'),
      this.db.query(`SELECT year, COUNT(*) count FROM legislations WHERE status IN ('PUBLISHED','AMENDED','REPEALED','SUSPENDED') GROUP BY year ORDER BY year DESC`),
    ]);
    const total = Number(countRows[0]?.total ?? 0);
    return { items, meta: { page, pageSize, total, pageCount: Math.ceil(total / pageSize) }, filters: { types, authorities, years } };
  }
  ...
  async detail(id: string) {
    const rows = await this.db.query(`
      ...
      WHERE l.id=? AND l.status IN ('PUBLISHED','AMENDED','REPEALED','SUSPENDED')`, [id]);
    if (!rows[0]) throw new NotFoundException('التشريع المطلوب غير موجود أو غير منشور.');
    return rows[0];
  }

  async structure(id: string) {
    await this.detail(id);
    return this.db.query(`SELECT id, parent_id parentId, node_type nodeType, label_ar labelAr, title_ar titleAr, sort_key sortKey
      FROM structure_nodes WHERE legislation_id=? ORDER BY sort_key`, [id]);
  }

  async articles(id: string, at?: string) {
    await this.detail(id);
    const effectiveDate = /^\d{4}-\d{2}-\d{2}$/.test(at ?? '') ? at! : new Date().toISOString().slice(0, 10);
    const rows = await this.db.query(`
      ...
      ORDER BY a.sort_key`, [effectiveDate, effectiveDate, id, effectiveDate, effectiveDate]);
    return { at: effectiveDate, items: rows };
  }

  async modifications(id: string) {
    await this.detail(id);
    const rows = await this.db.query(`SELECT am.id amendmentId,am.title_ar amendmentTitle,
      ...
      ORDER BY am.effective_from DESC,ao.application_order`,[id]) as Array<Record<string,unknown>>;
    const amendmentMap = new Map<string,{id:string;titleAr:string;issueDate:string;effectiveFrom:string;year:number;sourceName:string;operations:Array<Record<string,unknown>>}>();
    for(const row of rows){const key=String(row.amendmentId);if(!amendmentMap.has(key))amendmentMap.set(key,{id:key,titleAr:String(row.amendmentTitle),issueDate:String(row.issueDate),effectiveFrom:String(row.effectiveFrom),year:Number(row.year),sourceName:String(row.sourceName),operations:[]});amendmentMap.get(key)!.operations.push({id:row.operationId,operationType:row.operationType,applicationOrder:row.applicationOrder,citationText:row.citationText,articleId:row.articleId,articleLabel:row.articleLabel,paragraphLocator:row.paragraphLocator,previousText:row.previousText,newText:row.newText});}
    const amendments=[...amendmentMap.values()];
    const years=[...new Set(amendments.map((item)=>item.year))].map((year)=>({year,count:amendments.filter((item)=>item.year===year).reduce((sum,item)=>sum+item.operations.length,0)}));
    return {years,amendments};
  }

  async annexes(id:string){await this.detail(id);return this.db.query(`SELECT ax.id,ax.annex_type annexType,ax.title_ar titleAr,ax.status,
    ...
    WHERE ax.legislation_id=? AND ax.status IN ('PUBLISHED','REPLACED','REPEALED') ORDER BY av.valid_from DESC`,[id]);}

  async relations(id:string){await this.detail(id);return this.db.query(`SELECT lr.id,lr.relation_type relationType,lr.scope_text scopeText,
    ...
    ORDER BY lr.effective_from DESC`,[id,id,id,id,id,id,id]);}

  async create(dto: CreateLegislationDto, role: string, username: string | undefined) {
    return this.db.transaction(async (m) => {
      ...
      await m.query(`INSERT INTO legislations
        ...
        VALUES (?,?,?,?,?,?,'DRAFT','UNKNOWN','D')`, [id, dto.typeId, dto.authorityId, dto.officialNumber ?? null, dto.year, dto.titleAr]);
      const actors = await m.query(`SELECT u.id FROM users u JOIN user_roles ur ON ur.user_id=u.id JOIN roles r ON r.id=ur.role_id
        WHERE r.code=? AND (? IS NULL OR u.username=?) LIMIT 1`, [role, username ?? null, username ?? null]);
      await m.query(`INSERT INTO audit_logs (id,actor_id,action,entity_type,entity_id,after_json,reason)
        VALUES (?,?,'CREATE_DRAFT','LEGISLATION',?,?,?)`, [randomUUID(), actors[0]?.id ?? null, id, JSON.stringify(dto), 'إنشاء مسودة عبر API']);
