...
export class AnnexesController{
  ...
  @Get(':id') @ApiOperation({summary:'بيانات ملحق وإصداراته'})
  async detail(@Param('id') id:string){
    const rows=await this.db.query(`SELECT ax.id,ax.legislation_id legislationId,ax.annex_type annexType,ax.title_ar titleAr,ax.status,
      ...
      WHERE ax.id=? AND l.status IN ('PUBLISHED','AMENDED','REPEALED','SUSPENDED') ORDER BY av.valid_from DESC`,[id]);
    if(!rows[0])throw new NotFoundException('الملحق غير موجود أو غير منشور.');
    return {id,titleAr:rows[0].titleAr,annexType:rows[0].annexType,status:rows[0].status,versions:rows};
  }

  @Get(':id/file') @ApiOperation({summary:'عرض أو تنزيل ملف الملحق دون كشف مساره المحلي'})
  async file(@Param('id') id:string,@Res({passthrough:true}) response:Response){
    const rows=await this.db.query(`SELECT af.storage_key storageKey,af.original_name fileName,af.media_type mediaType
      ...
      ORDER BY av.valid_from DESC LIMIT 1`,[id]);
    if(!rows[0])throw new NotFoundException('لا يوجد ملف منشور لهذا الملحق.');
    const root=resolve(process.env.DATA_ROOT??resolve(process.cwd(),'../../data'));
    const target=resolve(root,String(rows[0].storageKey));
    if(!target.startsWith(`${root}${sep}`))throw new NotFoundException('مسار الملف غير صالح.');
    response.setHeader('Content-Type',String(rows[0].mediaType));
    response.setHeader('Content-Disposition',`inline; filename*=UTF-8''${encodeURIComponent(String(rows[0].fileName))}`);
