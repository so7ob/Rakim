...
export class ArticlesController {
  ...
  @Get(':id') @ApiOperation({ summary: 'المادة ونسختها النافذة الآن' })
  async article(@Param('id') id: string) { return this.at(id); }

  @Get(':id/at') @ApiOperation({ summary: 'نسخة المادة النافذة في تاريخ محدد' })
  async at(@Param('id') id: string, @Query('date') date?: string) {
    const at = /^\d{4}-\d{2}-\d{2}$/.test(date ?? '') ? date! : new Date().toISOString().slice(0, 10);
    const rows = await this.db.query(`SELECT a.id, a.legislation_id legislationId, a.current_label currentLabel,
      ...
      AND av.valid_from <= ? AND (av.valid_to IS NULL OR av.valid_to > ?) LIMIT 1`, [id, at, at]);
    if (!rows[0]) throw new NotFoundException('لا توجد نسخة نافذة للمادة في التاريخ المحدد.');
    return { at, ...rows[0] };
  }

  @Get(':id/versions') @ApiOperation({ summary: 'كل نسخ المادة التاريخية' })
  async versions(@Param('id') id: string) {
    return this.db.query(`SELECT av.id, av.version_no versionNo, av.text_original textOriginal,
      ...
      WHERE av.article_id=? ORDER BY av.valid_from DESC`, [id]);
  }

  @Get(':id/previous-texts') @ApiOperation({ summary: 'النصوص السابقة للمادة' })
  async previous(@Param('id') id: string) {
    return this.db.query(`SELECT av.id, av.version_no versionNo, av.text_original AS \`fullText\`,
      DATE_FORMAT(av.valid_from,'%Y-%m-%d') validFrom, DATE_FORMAT(av.valid_to,'%Y-%m-%d') validTo,
      ...
      ORDER BY av.valid_from DESC`, [id]);
  }
}
