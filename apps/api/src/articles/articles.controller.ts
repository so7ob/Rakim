...
export class ArticlesController {
  ...
  async at(@Param("id") id: string, @Query("date") date?: string) {
    ...
    const rows = await this.db.query(
      ...
      WHERE a.is_active=TRUE AND a.deleted_at IS NULL AND a.id=? AND l.is_active=TRUE AND l.deleted_at IS NULL AND l.status IN ('PUBLISHED','AMENDED','REPEALED','SUSPENDED')
  ...
  async versions(@Param("id") id: string) {
    return this.db.query(
      ...
      WHERE a.is_active=TRUE AND a.deleted_at IS NULL AND av.article_id=?
