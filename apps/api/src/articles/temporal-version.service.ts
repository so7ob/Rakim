...
export interface NewArticleVersion {
  ...
  status?: "DRAFT" | "PUBLISHED" | "REPEALED" | "FUTURE";
}
...
export class TemporalVersionService {
  ...
  async create(input: NewArticleVersion): Promise<string> {
    return this.db.transaction(async (manager) => {
      const article = await manager.query(
      ...
      if (overlap[0])
        throw new TemporalConflictError("ARTICLE_VERSION_PERIOD_OVERLAP");
      ...
      await manager.query(
        ...
        [
          id,
          input.articleId,
          Number(versionRows[0].nextVersion),
          input.textOriginal,
          input.textOriginal,
          normalizeArabic(input.textOriginal),
          input.validFrom,
          input.validTo ?? null,
          input.status ?? "DRAFT",
