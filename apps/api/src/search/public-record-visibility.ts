...
export const PUBLIC_INDEX_RECORD = `
 ...
 AND (sd.entity_type<>'RELATION' OR EXISTS (SELECT 1 FROM legal_relations visible_lr WHERE visible_lr.id=sd.entity_id AND visible_lr.is_active=TRUE AND visible_lr.deleted_at IS NULL AND visible_lr.review_status='PUBLISHED'))
